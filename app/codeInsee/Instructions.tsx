...
export const Instructions = () => {
  const instructions = (
    <>
      Cette Vue permet d'indiquer le code INSEE de la collectivité correspondant
      à chaque ligne.
      <br />
      Fonctionnement :
      <ul>
        <li>
          Indiquer la colonne correspondant au nom de la collectivité ainsi que
          la colonne à remplir pour les codes INSEE ou SIREN (colonne de type
          Texte). Il est possible d'indiquer d'autres colonnes pour aider à
          désambiguïser. Si la collectivité est un EPCI, c'est un code SIREN qui
          sera renseigné.
        </li>
        <li>
          Si vous n'avez pas indiqué de colonne permettant de désambiguïser la
          maille de la collectivité à rechercher, vous pouvez la définir
          globalement via la liste déroulante appropriée.
        </li>
        <li>
      ...
      <br />
      Voici la liste des <b>mailles</b> valides dans l'API interrogée et le type
      de code qui lui correspond. Si une maille est indiquée mais ne correspond
      à aucune présente dans cette liste elle sera alors ignorée.
      <br />
      <table cellSpacing="0">
        <thead>
          <tr>
        ...
        <tbody>
          {Object.values(DECOUPAGE_ADMIN).map((maille) => {
            const acceptedValues =
              maille.key in MAILLE_ACCEPTED_VALUES
                ? MAILLE_ACCEPTED_VALUES[
                    maille.key as keyof typeof MAILLE_ACCEPTED_VALUES
                  ]
                : [];
            return (
              <tr key={maille.key}>
