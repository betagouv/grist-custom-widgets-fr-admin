...
const Sirene = () => {
  ...
  const [geocodeResults, setGeocodeResults] = useState<GeocodeResults | null>(
    null,
  );
  const [currentRecord, setCurrentRecord] = useState<RowRecord | null>(null);
  const [currentMappings, setCurrentMappings] =
    useState<WidgetColumnMap | null>(null);
  const [selectedChoice, setSelectedChoice] = useState<GeocodeChoice | null>(
    null,
  );

  // Derived state
  const choices = geocodeResults?.entreprises
    ? geocodeResults.entreprises.map((result) => ({
      libelle: result.simpleLabel,
      siren: result.siren,
      code_commune:
        result.firstMatchingEtablissement.codeCommuneEtablissement,
      siret: result.firstMatchingEtablissement.siret,
      score: result.score,
    }))
    : null;
  const currentRecordColumns = currentRecord
    ? grist.mapColumnNames(currentRecord)
    : null;
  ...
  useEffect(() => {
    ...
    if (!currentRecordColumns) {
      setMessage([
        "Configurez d’abord les colonnes source et destination dans les options du widget.",
        "error",
      ]);
      return;
    }
    ...
    if (currentRecordColumns.siren) {
      setMessage([
        `Code déjà présent pour "${currentRecordColumns.nom}" : ${currentRecordColumns.siren}.`,
      ]);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (choices.length === 0) {
      setMessage([
        `Pas de résultat pour "${currentRecordColumns?.nom}"`,
        "warning",
      ]);
      return;
    }
    ...
    grist
      ...
      .then((tableId) => {
        grist.docApi.applyUserActions([
          ["UpdateRecord", tableId, currentRecord.id, patchObject],
        ]);
      });
  };
        ...
        <div id="choice">
          <form onSubmit={applySelectedChoice}>
            ...
            <button type="submit" disabled={!selectedChoice}>
              Utiliser le résultat sélectionné
            </button>
          </form>
        </div>
      )}
