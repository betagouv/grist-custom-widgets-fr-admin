...
const AgentExpenseWidget = () => {
    ...
    const loadPdf = async () => {
        if (!gristData?.mappings[COLUMN_MAPPING_NAMES.PDF_INPUT.name] ||
            !gristData.records[0][
            gristData.mappings[COLUMN_MAPPING_NAMES.PDF_INPUT.name] as keyof typeof gristData.records[0]
            ]
        ) {
            ...
            if (gristData?.records[0] && gristData.mappings) {
                ...
                for (const [key, config] of Object.entries(COLUMN_MAPPING_NAMES)) {
                    ...
                    try {
                        ...
                        if (["BILLET_MISSIONE", "TRANSPORT_COMMUN", "HOTEL_MISSIONE"].includes(key)) {
