...
import {
  extractIndicateurValue,
  extractRecordNumber,
} from "./lib/indicateurExtractor";
...
export const MultiColonneView = ({
  tokenInfo,
  tableId,
  records,
  setFeedback,
  setGlobalError,
}: MultiColonneViewProps) => {
  ...
  const handleUpdateSelectedColumns = async () => {
    ...
    const indicateursIdentifiants = columnsToUpdate.map(
      (col) => col.insituIndicateurId,
    );
  ...
  const writeDataInTable = (
    ...
    const dataByRecord: {
      [recordId: string]: { [columnId: string]: number | string };
    } = {};
    ...
    dataFromApi.forEach((resultatIndicateur) => {
      ...
      if (!identifiantIndicateur) {
        console.warn(
          "Indicateur sans identifiant ignoré",
          resultatIndicateur.metadata,
        );
        return;
      }

      // Trouver la colonne correspondant à cet indicateur
      const colonne = columnsToUpdate.find(
        (col) => col.insituIndicateurId === identifiantIndicateur,
      );
      if (!colonne) {
        return;
      }

      const afficherDetailIndicateur =
        wantIndicateursDetail[colonne.id] === true;

      // Parcours de toutes les mailles (records) pour cet indicateur
      Object.entries(resultatIndicateur.mailles).forEach(
        ([recordId, indicateur]) => {
          const valeurIndicateur = extractIndicateurValue(
            indicateur,
            afficherDetailIndicateur,
          );
      ...
      try {
        ...
        const filtered = data.columns
          ...
          .map((col: any) => ({
            ...
            insituIndicateurId: col.fields.description.split(
        ...
        setGetColumnsError(
          "Impossible de récupérer les colonnes de la table, essayez de recharger la page",
        );
        setLoading(false);
      }
    };
    ...
    <>
      ...
      {filteredColumns.length === 0 ? (
        ...
        <div className="multi-colonne">
          ...
          <p>
            Sélectionnez les colonnes que vous souhaitez mettre à jour et
            indiquez si vous souhaitez récupérer le décompte de l'indicateur
            (Ex: Nombre de villes concernées par le programme) ou le détail de
            l'indicateur (Ex : Liste des villes concernées par le programme)
          </p>
          <table>
            <thead>
              <tr>
